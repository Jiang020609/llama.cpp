...
- `--diffusion-mbsd-lifecycle-bookkeeping`: Observer-only token/cache lifecycle bookkeeping for dense MBSD. Combine it with staged stabilization to classify MBSD tokens by the visibility and stability thresholds (default: disabled).
- `--diffusion-mbsd-logical-kv`: Host-only transactional stable, mutable, and step-local logical KV bookkeeping layered on the dense MBSD lifecycle reference (default: disabled).
...
`--diffusion-mbsd-lifecycle-bookkeeping` adds a host-only observer to that same dense execution. When combined with `--diffusion-staged-token-stabilization`, it reuses confidences already sampled by MBSD to classify each generated position as invisible, visible, or stable. These states are monotonic. Invisible positions remain mask tokens, visible positions remain mutable across block boundaries, and stable positions freeze their semantic token and planned KV version. A low-confidence token that the unchanged MBSD schedule already commits is recorded as forced visible rather than being left invisible; this compatibility rule preserves exact output and forward parity. Stability is additionally limited to the resolved left-to-right prefix before the first EOG so later mask resolution or EOG-tail canonicalization cannot mutate a stable token.

Each position also records its static owner block, dynamic `prefix`, `current`, or `future-draft` role, token and planned cache versions, latest confidence, last refresh step, and refresh-pending state. A future draft can retain prediction progress but remains invisible, never performs a semantic commit, and never enters planned prefix cache residency. Invisible maps to no residency, visible maps to mutable residency, and stable maps to planned long-term-stable residency. Block completion changes roles but does not promote every visible token to stable.

Visible prefix positions enter an observer-only refresh queue. The queue records work that a later sparse-KV implementation would need; it does not execute that work, so `last_refresh_step` remains -1. The observer never supplies a token, KV entry, position, logit row, or batch row to model execution. It performs no cache read, write, refresh, merge, physical compaction, or asynchronous task. Diagnostics require invisible count to equal remaining mask count at every step, future-draft semantic and prefix-cache commits to remain zero, and generated token IDs, ID hash, MBSD trajectory hash, main-forward count, transformer rows, and logit rows to match the same dense MBSD run exactly. Actual row saving remains zero, and state, ownership, and refresh-queue hashes must be deterministic.

`--diffusion-mbsd-logical-kv` adds the Phase 2 host-only cache lifecycle model on top of MBSD, lifecycle bookkeeping, and staged stabilization. It requires all three flags. It is mutually exclusive with fresh KV, compact MBSD, prefix KV, and the full-sequence KV oracle. The transformer still executes the same no-KV full-sequence reference.

The logical cache has three tiers. The long-term tier admits stable positions and never mutates an admitted entry. Its stored role is the admission-time role; the existing lifecycle continues to hold the dynamic role after later block transitions. The mutable tier admits visible positions and can replace an entry only in an atomic step-boundary transaction whose token and cache versions still match the live lifecycle state. A visible entry can remain mutable after its owner block becomes a prefix block and can later move to the stable tier. The step-local tier is rebuilt from the current MBSD input window before each forward and records the current-window state. At the boundary it is synchronized with every valid future draft, including newly predicted drafts and drafts outside the next input window. This is host scheduler state, not a reusable or physical KV tensor. Every future draft has exactly one step-local logical entry and no durable entry.

Block completion and logical sealing are separate events. A committed block containing visible or invisible positions is not sealed. It becomes logically sealed only after every position is stable and present in the stable logical cache. Stable carryover checks, boundary-only mutation checks, durable ownership checks, version checks, seal refusals, and deterministic cache-trajectory hashes make this distinction observable. A host-only stale-update self-test submits an older version and requires it to be dropped without changing the mutable entry.

This mode plans cache residency but performs no physical cache read, write, refresh, merge, compact batch, asynchronous task, or row saving. It is a prerequisite for the later sparse refresh and step-boundary physical merge, not their implementation.
...
`--diffusion-mbsd-compact` is reserved for a later paper-alignment stage. It is rejected during parameter validation, before model loading, until the host-only stable and mutable plan is connected to correct physical sparse refresh and step-boundary merge. There is no single-block fallback and no silent substitution with either reference mode.
...
The no-KV MBSD reference, fresh-KV correctness oracle, and logical-KV ledger are diagnostics, not the paper's final execution path. The logical ledger models stable and mutable residency on the host, but no mode here executes sparse refresh, delayed physical cache merge, or the asynchronous CPU/NPU dual path. No physical compact transformer execution is active in this stage.
...
```
llama-diffusion-cli -m dream7b.gguf -p "write code to train MNIST in pytorch" -c 128 -b 128 -ub 128 --temp 0 --diffusion-block-length 32 --diffusion-generated-block-schedule --diffusion-mbsd --diffusion-mbsd-lifecycle-bookkeeping --diffusion-staged-token-stabilization --diffusion-visibility-threshold 0.7 --diffusion-stability-threshold 0.9 --diffusion-mbsd-trigger 4 --diffusion-mbsd-lookahead 32 --diffusion-dump-generated-tokens --diffusion-algorithm 4 --diffusion-alg-temp 0 --diffusion-steps 32
```

#### Dream MBSD logical KV bookkeeping:
```
llama-diffusion-cli -m dream7b.gguf -p "write code to train MNIST in pytorch" -c 128 -b 128 -ub 128 --temp 0 --diffusion-block-length 32 --diffusion-generated-block-schedule --diffusion-mbsd --diffusion-mbsd-lifecycle-bookkeeping --diffusion-mbsd-logical-kv --diffusion-staged-token-stabilization --diffusion-visibility-threshold 0.7 --diffusion-stability-threshold 0.9 --diffusion-mbsd-trigger 4 --diffusion-mbsd-lookahead 32 --diffusion-dump-generated-tokens --diffusion-algorithm 4 --diffusion-alg-temp 0 --diffusion-steps 32
```
...
The normal repeated matrix also compares dense lookahead-32 MBSD with the combined lifecycle and staged observer and requires exact token, trajectory, forward, transformer-row, and logit-row parity. Set `LIFECYCLE_MATRIX=1` for the full lifecycle and logical-KV gate across multiple prompts, three seeds, and both single-block and multi-block shapes. The gate compares dense reference, lifecycle observer, and logical-KV observer token IDs, ID hash, trajectory, commitment order, draft decisions, forwards, transformer rows, and logit rows. It writes lifecycle and logical-KV summaries, checks all state/cache invariants, requires zero actual cache or row-saving work, exercises stale-version rejection, and repeats representative runs to verify deterministic lifecycle and cache hashes.

A successful full matrix prints `PHASE1_REGRESSION_GATE=PASS`, `LOGICAL_KV_PARITY_GATE=PASS`, `LOGICAL_KV_INVARIANT_GATE=PASS`, `LOGICAL_KV_COVERAGE_GATE=PASS`, and `PHASE2_LOGICAL_KV_GATE=PASS`. These gates validate the host-only plan. They do not claim that physical compact KV or Apple-specific execution is complete.

When tests are enabled, `test-diffusion-logical-kv` runs the model-free stale-version rejection twice. It requires the older update to be dropped without changing the newer mutable entry and prints `LOGICAL_KV_STALE_SELF_TEST=PASS`.
